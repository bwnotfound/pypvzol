...
class UpgradeSkillWidget(QWidget):
    ...
    def init_ui(self):
        ...
        self.pool_size_combobox.setCurrentIndex(
            self.usersettings.skill_man.pool_size - 1
        )
    ...
    def pool_size_combobox_current_index_changed(self):
        self.usersettings.skill_man.pool_size = (
...
class UpgradeSkillThread(Thread):
    ...
    def upgrade_plant_skill(self, plant: Plant, skill):
        ...
        if int(skill['id']) == plant.special_skill_id:
            is_spec = True
        result = self.usersettings.skill_man.upgrade_skill(plant.id, skill, is_spec)
        if result is None:
            return False
    ...
    def run(self):
        try:
            self.pool = concurrent.futures.ThreadPoolExecutor(
                max_workers=self.usersettings.skill_man.pool_size
            )
            self.upgrade()
        finally:
