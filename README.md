...
该助手支持私服的使用，但是官服适配还未完成，等私服适配完成后再适配官服。
...
1.  能够刷新用户面板的数据
2.  植物锁定（同时提供筛选合成池中其他属性过高的植物功能）
3.  添加一键指令：打洞、jjc。例如jjc就是允许指令的话就一键打，不能再一个个打
4.  把带级和刷洞区分一下
5.  自动速度复合
6.  一键上下植物的领地。每次打完了领地就自动下，每次循环自动下植物
7.  多线程刷品加速
8.  自动复合加入是否全传给主力的选项。自动复合优化ui，分隔按钮
9.  加入自动检测合成和复合的只能机制，能够在检测到数据有异常时发出弹窗警告
10. 添加一键设置所有账号的延时时间功能
11. 自动开魔神，选择是否刷无极，然后带级并自动复合
12. (可选)云端助手，提供服务器选项，可以搭载到云端然后远程使用，类似远程操控
13. (可选)加默认模板和种子文件识别功能
14. 多个账号共同设置除了打洞之外的功能
15. 日志本地存储
16. 跨服次数限制到多少次为止
...
2. 恶性bug：pyqtSignal在多线程情况是异步的，因此调用emit后立马进行下一步数据处理。
        导致signal->refresh_all时，refresh所用数据被多线程所更改而报错。
3. 复合闪退后内置合成池仍有植物。加入检测机制或者reset按钮