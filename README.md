...
2.  自动进化（可以自主设置进化路线）
3.  自动刷品（支持多个植物同时刷品，但请一次选取多个，否则在处理完上一批刷品前是不会处理下一批的）。该功能因为 api 问题以及服务器有概率不响应请求（在快速请求的条件下），所以如果刷品是有可能不会在魔神处终止，而是在耀世或不朽终止（前提要有书）。建议进化到魔神终止。
...
4.  自动合成没有很好的检测到数据不一致(有人传承后刷新仓库崩了)
5.  日常任务领取bug
6.  除了普通物品可以卖，箱子其实也是可以卖的
...
13. 自动升级技能
14. 挑战战利品展示
15. 处理超时请求或者长时间(5s以上)未响应请求(允许用户设置最大延长时间)
16. 自动合成加入特书
17. 洞口练级添加智能判断当前花园层数(私服)，智能爬取好友不同层数的洞口
18. 可以选择跳过回血检测
19. 打竞技场
20. 考虑添加远程端口能够简单控制助手